...
void Card::setActualPoints(int ap) {
	actualPoints = ap;
}
...
struct myclass {
	bool operator() (Card i, Card j) { return (i.getPoints() < j.getPoints()); }
} myobject;
...
int Deck::getMeldSize() {
	return run.size() + set.size();
}
...
Card Deck::removeCardAt(int num) {
	...
	if ((set.size() + run.size()) > num) {
		Card card = run[num - set.size()];
		run.erase(run.begin() + (num - set.size()));
		return card;
	} else {
		num = num - (set.size() + run.size());
	 	Card card = deck[num];
...
void Deck::scanRun()
{
	...
	for (unsigned int i = 0; i < deck.size(); i++) //takes just one card
	{
		for (j = i + 1; j < deck.size(); j++) //compares each cards point value with the rest
		{
			...
			{
				count++;
				if (deck[i].getPoints() + count == deck[j].getPoints())
				{
					countRun++;
...
void Deck::populateDeck()
{
	...
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 13; j++) {
			...
			Card card(suit[i], value[j], point[j]);
			card.setActualPoints(actualPoints);
			deck.push_back(card);
		}
	}
}
		...
		while (!roundFinished) {
			...
			if (turn == 2) {
				...
				if (AIHand.hasMelds()) {
					...
					int discardCard = random(AIHand.getMeldSize(), 10);
					discardPile.addCard(AIHand.removeCardAt(discardCard));
				}
			}
